...
type AddressPort struct {
	Address netip.Addr `yaml:"address" edge:"address,omitempty"` // @TODO I think this can also be 1.2.3.4-5.6.7.8 style, so maybe needs to be string or custom type (at least for NAT)
...
type NetworkPort struct {
	Address netip.Prefix `yaml:"address" edge:"address,omitempty"` // @TODO I think this can also be 1.2.3.4-5.6.7.8 style, so maybe needs to be string or custom type (at least for NAT)
	Port    uint16       `yaml:"port" edge:"port,omitempty"`
}
